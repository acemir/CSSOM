					...
					case "selector":
						...
						while (ancestorRules.length > 0) {
							...
							if (parentRule instanceof CSSOM.CSSGroupingRule && (parentRule.constructor.name !== 'CSSStyleRule' || parentRule.__parentRule)) {
								// Skip if parentRule is the same as currentScope (we just popped the rule being closed itself)
								if (parentRule === currentScope) {
									continue;
								}
								
								if (nestedSelectorRule) {
									// Check if we're closing the nestedSelectorRule itself or a direct container of it
									// vs closing a nested grouping rule inside nestedSelectorRule (like nested at-rule)
									if (currentScope === nestedSelectorRule || nestedSelectorRule.__parentRule === currentScope) {
										...
										// We're closing a nested grouping rule inside nestedSelectorRule
										// (e.g., at-rule inside at-rule inside styleRule)
										// Add currentScope to parentRule and break
										prevScope = currentScope;
