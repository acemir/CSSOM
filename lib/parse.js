...
CSSOM.parse = function parse(token, opts, errorHandler) {
	...
	*/
	...
	function normalizeAttributeContent(content) {
		...
		if (firstChar === '"') {
			// Double quote opened - must be closed
			if (trimmedValue.length < 2 || trimmedValue[trimmedValue.length - 1] !== '"') {
				return null; // Unclosed double quote - invalid
			}
			// Extract inner value and re-quote
			var innerValue = trimmedValue.slice(1, -1);
			return attrName + operator + '"' + innerValue + '"' + (flag ? ' ' + flag : '');
		}
		
		if (firstChar === "'") {
			// Single quote opened - must be closed
			if (trimmedValue.length < 2 || trimmedValue[trimmedValue.length - 1] !== "'") {
				return null; // Unclosed single quote - invalid
			}
			// Extract inner value and convert to double quotes
			var innerValue = trimmedValue.slice(1, -1);
			return attrName + operator + '"' + innerValue + '"' + (flag ? ' ' + flag : '');
		}
		
		// Check for invalid characters in unquoted values
		// Special characters must be either quoted or escaped
		if (specialCharsNeedEscapeRegExp.test(trimmedValue)) {
			return null; // Special characters not allowed in unquoted attribute values
		}
		
		// Unquoted value - add double quotes
		return attrName + operator + '"' + trimmedValue + '"' + (flag ? ' ' + flag : '');
	}
