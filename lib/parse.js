	...
	for (var character; (character = token.charAt(i)); i++) {
		...
		case "{":
			...
			if (state === "selector" || state === "atRule") {
				...
				const parts = buffer.trim().split(/\s+to\s+/);
				const hasStart = parts[0] &&
					parts[0].indexOf('(') === 0 &&
					parts[0].indexOf(')') === parts[0].length - 1;
				const hasEnd = parts[1] &&
					parts[1].indexOf('(') === 0 &&
					parts[1].indexOf(')') === parts[1].length - 1;

				if (hasStart) {
					scopeRule.__start = parts[0].slice(1, -1);
				}
				if (hasEnd) {
					scopeRule.__end = parts[1].slice(1, -1);
				}
				...
				if (!hasStart &&
					...
					scopeRule.__end = parts[0].slice(4, -1);
				}
