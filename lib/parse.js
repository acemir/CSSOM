...
CSSOM.parse = function parse(token, opts, errorHandler) {
	...
	*/
	...
	// Regex helpers fot selector identifier validation
	var endsWithHexEscapeRegExp = /\\[0-9a-fA-F]{1,6}\s$/; // Matches selector ending with hex escape + space
	var endsWithEscapeDigitRegExp = /\\[^0-9a-fA-F\s][0-9]\s$/; // Matches selector ending with non-hex escape + digit + space
	...
	function normalizeSelectorEscapes(selector) {
		...
		while (i < selector.length) {
			...
			if (char === '.' || char === '#') {
				...
				if (isHexDigit(nextChar) || isWhitespaceChar(nextChar)) {
					result += ' ';
				}
				lastWasHexEscape = true;
				needsEscapeForIdent = false;
	...
	function decodeEscapeSequence(escapeSeq) {
		...
		var hexMatch = content.match(/^([0-9a-fA-F]{1,6})[ \t\r\n\f]?/);
		if (hexMatch) {
		...
		switch (character) {
			...
			case "{":
				...
				if (state === "selector" || state === "atRule") {
					...
					// Don't trim if selector ends with escape sequence (preserve terminating space)
					// or if it ends with a digit after an escape (which will become hex escape + space)
					var bufferToProcess = buffer;
					var endsWithEscape = endsWithHexEscapeRegExp.test(buffer);
					var endsWithEscapeThenDigit = endsWithEscapeDigitRegExp.test(buffer);
					if (!endsWithEscape && !endsWithEscapeThenDigit) {
						bufferToProcess = buffer.trim();
					}
					var processedSelectorText = processSelectorText(bufferToProcess);
