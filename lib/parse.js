...
/**
	 * Validates CSS selector syntax (loose validation).
	 * 
	 * Supports:
	 ...
	 * - Nesting selector (&)
	 * Note: This is a "loose" validation - it doesn't catch all invalid selectors
	 * but is fast and handles common cases correctly.
	 * 
	 * Reordered to check common patterns first
	 */
	var looseSelectorRegExp = /^(\.(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])+|#(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])+|::?(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])+(?:\((.*)\))?|&|\s+|\s*[>+~]\s*|\[(?:[^\[\]'"]|'(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*")*(?:\s+[iI])?\]|(?:(?:\*|(?:\\.|[a-zA-Z_\u00A0-\uFFFF])(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])*|)\|)?(?:\\.|[a-zA-Z_\u00A0-\uFFFF])(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])*|(?:(?:\*|(?:\\.|[a-zA-Z_\u00A0-\uFFFF])(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])*|)\|)?\*)+$/;

	// Pattern breakdown (cannot use comments inside regex in JS):
	// 1. \.(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])+              → Class selector
	// 2. #(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])+               → ID selector
	// 3. ::?(?:\\.|[a-zA-Z0-9_\u00A0-\uFFFF\-])+(?:\(.*\))?  → Pseudo-class/element
	// 4. &                                                    → Nesting selector
	// 5. \s+                                                  → Whitespace (descendant)
	// 6. \s*[>+~]\s*                                          → Combinators (>, +, ~)
	// 7. \[...\]                                              → Attribute selector
	// 8. (?:...\|)?(?:\\.|[a-zA-Z_\u00A0-\uFFFF])...          → Type selector with namespace
	// 9. (?:...\|)?\*                                         → Universal selector with namespace
