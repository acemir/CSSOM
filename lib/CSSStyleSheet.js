...
CSSOM.CSSStyleSheet.prototype.insertRule = function(rule, index) {
	...
	if (parsedSheet.cssRules.length !== 1) {
		...
		errorUtils.throwParseError(this, 'insertRule', this.constructor.name, ruleToParse, domExceptionName);
	}
...
CSSOM.CSSStyleSheet.prototype.deleteRule = function(index) {
	...
	if (this.cssRules[index] && this.cssRules[index].constructor.name == "CSSNamespaceRule") {
		...
		if (!shouldContinue) {
			errorUtils.throwError(this, 'DOMException', "Failed to execute 'deleteRule' on '" + this.constructor.name + "': Failed to execute 'deleteRule' on 'CSSStyleSheet': Failed to delete rule", "InvalidStateError");
		}
	}
